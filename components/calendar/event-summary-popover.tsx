...
export function EventSummaryPopover() {
  const { isEventSummaryOpen, closeEventSummary, selectedEvent } = useEventStore();

  if (!isEventSummaryOpen || !selectedEvent) return null;
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      />
      <Sheet open>
        <SheetContent side="right" onEscapeKeyDown={closeEventSummary} className="min-w-[500px] w-full">
          <SheetHeader>
          ...
          <SheetFooter>
            <Button variant="outline" onClick={closeEventSummary}>Close</Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
    </div>
